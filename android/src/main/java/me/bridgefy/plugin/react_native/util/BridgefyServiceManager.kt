...
class BridgefyServiceManager private constructor(context: Context) {
    ...
    fun isSDKInitialized(): Boolean {
        return cachedIsInitialized ?: prefs.getBoolean(KEY_SDK_INITIALIZED, false).also {
    ...
    fun isSDKStarted(): Boolean {
        return cachedIsStarted ?: prefs.getBoolean(KEY_SDK_STARTED, false).also {
            cachedIsStarted = it
    ...
    fun getCurrentUserId(): String? {
        return cachedUserId ?: prefs.getString(KEY_CURRENT_USER_ID, null).also {
            cachedUserId = it
    ...
    fun getServiceStatus(): String {
        return prefs.getString(KEY_SERVICE_STATUS, "unknown") ?: "unknown"
    }
