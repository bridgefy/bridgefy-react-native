...
export const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    const eventEmitter = new NativeEventEmitter(Platform.OS==='ios' ? NativeModules.BridgefyReactNative : null);
    subscriptions.push(
    ...
    subscriptions.push(
      eventEmitter.addListener(
        BridgefyEvents.bridgefyDidStop,
        () => {
    ...
    subscriptions.push(
      eventEmitter.addListener(
        ...
        ({data, messageId, transmissionMode}:IDidReceiveDataOut) => {
          ...
          addLog({
            text: `didReceiveData: ${data}, ${messageId}, ${JSON.stringify(transmissionMode)}`,
            type: LogType.normal,
          });
        }
      )
