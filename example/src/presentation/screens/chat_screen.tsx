        ...
        <FlatList
          ...
          onContentSizeChange={() => {
            if (messageList.length>1)
              scrollMessageList?.current?.scrollToIndex({index: (messageList.length-2), animated: true});
          }}
          ...
          renderItem={({item})=>(
            <View style={[
              ...
              {
                alignSelf: item.origin===OriginMessage.me ? 'flex-end' : 'flex-start',
                backgroundColor: item.origin===OriginMessage.me ? globalColors.primaryColor : globalColors.secondaryColor,
              }
            ]}>
              <Text
                style={{
